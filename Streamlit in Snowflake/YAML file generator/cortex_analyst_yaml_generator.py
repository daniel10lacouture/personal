...
def show_table_definition_page():
    ...
    if st.button("Add Table to YAML"):
        if len(st.session_state['tables']) < 3:
            ...
            yaml_str = yaml.dump(st.session_state['yaml_structure'], sort_keys=False, indent=2)
            st.session_state['yaml_str'] = yaml_str  # Save to session state

    # Display the updated YAML structure
    st.code(st.session_state.get('yaml_str', yaml.dump(yaml_template, sort_keys=False, indent=2)), language='yaml')
...
def main():
    ...
    page = st.sidebar.radio("Go to", ["Welcome", "Get Started", "Table Definition", "Reset"])
